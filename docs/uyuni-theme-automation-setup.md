...
이 문서는 Uyuni Keycloak 테마 자동화 설정을 위한 간단한 가이드입니다. uyuni-login-theme 레포지토리에서 테마가 변경될 때마다 Docker 이미지가 자동으로 빌드되고, astrago-deployment에서는 최신 이미지를 사용합니다.
...
#### 1.2 수정된 파일들
- **`applications/keycloak/values.yaml.gotmpl`**: Keycloak 이미지 설정 추가
...
1. **uyuni-login-theme**: 테마 파일 관리 및 Docker 이미지 빌드/푸시
2. **Docker Hub**: xiilab/astrago-keycloak 이미지 저장소
3. **astrago-deployment**: 최신 Docker 이미지 사용
...
#### 1.2 GitHub Actions 워크플로우 추가

uyuni-login-theme 레포지토리에 `.github/workflows/build-keycloak-image.yml` 파일을 생성하세요:
...
on:
  push:
    branches: [ "feature/keycloak-astrago-theme" ]
  workflow_dispatch:
    ...
    steps:
      ...
      - name: Build and push Docker image
        ...
        with:
          ...
          push: true
          tags: |
            xiilab/astrago-keycloak:latest
...
- `latest` 태그 또는 환경별 태그 사용
- `Always` pull policy 사용
...
### 1. 테마 변경 감지
- uyuni-login-theme의 `feature/keycloak-astrago-theme` 브랜치에서 테마 파일 변경
- GitHub Actions 워크플로우 자동 실행

### 2. Docker 이미지 빌드 및 푸시
- Dockerfile.keycloak로 이미지 빌드
- Docker Hub에 `xiilab/astrago-keycloak:latest` 및 `xiilab/astrago-keycloak:a1b2` (4자리 커밋 해시) 푸시
...
2. 변경사항 push
3. GitHub Actions에서 이미지 빌드 확인
4. Docker Hub에서 새 이미지 확인
...
- **uyuni-login-theme**: `feature/keycloak-astrago-theme` 브랜치에서만 이미지 빌드
- **astrago-deployment**: 
